...
std::vector<byte> AESCipher::encrypt(const byte* plaintext, size_t plaintextLength) {
    int cipherTextLength = plaintextLength + EVP_CIPHER_CTX_block_size(ctx.get());
...
std::vector<byte> AESCipher::decrypt(const byte* ciphertext, size_t ciphertextLength) {
    ...
    if (!EVP_DecryptInit_ex(ctx.get(), nullptr, nullptr, key.data(), iv.data()) ||
        !EVP_DecryptUpdate(ctx.get(), plaintext.data(), &outLength1, ciphertext, ciphertextLength)) {
...
std::string AESCipher::base64Encode(const std::vector<byte>& data) {
    ...
    typedef base64_from_binary<transform_width<std::vector<byte>::const_iterator, 6, 8>> base64_enc;
    std::copy(base64_enc(data.begin()), base64_enc(data.end()), ostream_iterator<char>(os));
